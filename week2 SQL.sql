...
select p.stud_id,p.stud_name,p.national_id,p.age,p.gender,p.phone_number,c.next_of_kin_name,c.next_of_kin_relation,c.next_of_kin_contacts,s.current_home_county,s.secondary_school_county,s.residence,f.sem_fee,f.fee_paid
from financial_details f
join school_details s
on f.stud_id = s.stud_id
join personal_details p
on s.stud_id = p.stud_id
join contact_details c
on p.phone_number = c.phone_number;
